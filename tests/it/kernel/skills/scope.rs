...
fn shared_skill_visible_to_everyone() {
    let skill = make_skill(SkillScope::Shared, "", None);
...
fn private_skill_visible_to_same_user() {
    let skill = make_skill(SkillScope::Private, "user-1", Some("user-1"));
    assert!(skill.is_visible_to("user-1"));
}

#[test]
fn private_skill_not_visible_to_other_user() {
    let skill = make_skill(SkillScope::Private, "user-1", Some("user-1"));
    assert!(!skill.is_visible_to("user-2"));
}

#[test]
fn private_skill_visible_to_same_user_and_creator() {
    let skill = make_skill(SkillScope::Private, "user-1", Some("user-1"));
    assert!(skill.is_visible_to("user-1"));
}

#[test]
fn private_skill_not_visible_to_different_user_same_creator() {
    let skill = make_skill(SkillScope::Private, "user-1", Some("user-1"));
    assert!(!skill.is_visible_to("user-2"));
}
