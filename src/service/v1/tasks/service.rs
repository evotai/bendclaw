...
use crate::kernel::task::management::CreateTaskParams;
use crate::kernel::task::management::UpdateTaskParams;
...
pub(super) async fn list_tasks(
    ...
    let limit = q.limit();
    let records = management::list_tasks(&pool, limit).await?;
...
pub(super) async fn create_task(
    ...
    let record = management::create_task(&pool, params).await?;
    Ok(record)
}
...
pub(super) async fn update_task(
    ...
    let record = management::update_task(&pool, task_id, params).await?;
    Ok(record)
}

pub(super) async fn delete_task(state: &AppState, agent_id: &str, task_id: &str) -> Result<String> {
    let pool = state.runtime.databases().agent_pool(agent_id)?;
    management::delete_task(&pool, task_id).await?;
...
pub(super) async fn toggle_task(
    ...
    let pool = state.runtime.databases().agent_pool(agent_id)?;
    Ok(management::toggle_task(&pool, task_id).await?)
}
...
pub(super) async fn list_task_history(
    ...
    let pool = state.runtime.databases().agent_pool(agent_id)?;
    let limit = q.limit();
    let records = management::list_task_history(&pool, task_id, limit).await?;
    let total = count_u64(
        &pool,
        &format!(
