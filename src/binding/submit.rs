//! Unified submit entry point for the main pipeline.
//!
//! All inbound paths (HTTP, channel, task, CLI) converge here.
//! `submit_turn()` is the single entry into: request → binding → execution.
...
// ── submit_turn — unified entry point ───────────────────────────────────────

/// Submit a user turn through the unified pipeline.
///
/// This is the **single entry point** for all inbound paths:
/// - HTTP (`server/v1/runs/service.rs`)
/// - Channel (`channels/ingress/submit_and_deliver.rs`)
/// - Task (`tasks/execution/task_runner.rs`)
/// - CLI (`cli/agent_cmd.rs`)
///
/// Flow: acquire session → classify control commands → submit turn to session.
...
pub async fn submit_turn(
    ...
    if let Some(command) = classify_control_command(&normalized) {
        diagnostics::log_control_command_classified(diagnostics::ControlCommandInfo {
            ...
            handled: true,
            handler: "binding.submit_turn",
        });

        return handle_control_command(runtime, command, agent_id, session_id, user_id).await;
    }
    ...
    let session =
        crate::sessions::factory::acquire_cloud_session(runtime, agent_id, session_id, user_id)
            .await?;
...
async fn handle_control_command(
    ...
    let message = match command {
        ...
        ControlCommand::Status => match runtime.sessions().get(session_id) {
            Some(ref s) => {
                let info = s.info();
