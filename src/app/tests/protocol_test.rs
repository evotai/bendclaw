...
#[test]
fn run_event_context_maps_all_protocol_events() {
    let ctx = RunEventContext::new("run-1", "sess-1", 1);

    // TurnStart -> TurnStarted
    let e = ctx.map(&ProtocolEvent::TurnStart);
    assert!(e.is_some());
    assert_eq!(e.unwrap().kind_str(), "turn_started");

    // AssistantDelta
    let e = ctx.map(&ProtocolEvent::AssistantDelta {
        delta: Some("x".into()),
        thinking_delta: None,
    });
    assert_eq!(e.unwrap().kind_str(), "assistant_delta");
    ...
    // ToolStart
    let e = ctx.map(&ProtocolEvent::ToolStart {
    ...
    // ToolEnd
    let e = ctx.map(&ProtocolEvent::ToolEnd {
        tool_call_id: "tc".into(),
        tool_name: "read".into(),
        content: "ok".into(),
        is_error: false,
    ...
    // AgentStart -> None
    assert!(ctx.map(&ProtocolEvent::AgentStart).is_none());

    // AgentEnd -> None
    assert!(ctx
        .map(&ProtocolEvent::AgentEnd {
            transcripts: vec![],
            usage: UsageSummary {
                input: 0,
                output: 0
